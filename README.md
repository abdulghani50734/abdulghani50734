...
import { Github, Linkedin, ArrowUpRight, Moon, Sun } from "lucide-react";
...
export default function Home() {
  ...
  return (
    <main className={dark? "dark bg-[#0A0A0A] text-white" : "bg-white text-black"}>
      {/* Navbar */}
      <nav className="fixed w-full z-50 px-6 py-4 backdrop-blur-xl border-b border-white/10">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <h1 className="font-bold text-xl font-[Poppins]">Awais.dev</h1>
          <button onClick={() => setDark(!dark)} className="p-2 rounded-full border border-white/20">
